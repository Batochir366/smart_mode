...
const MOCK_SMART_MODE_PHRASES = ['Make it simple.']

const heroTitleClass =
  'font-benzin text-[clamp(2.75rem,8.5vw,6.25rem)] font-bold leading-[0.95] text-white'
...
const heroBadge = (
  <p className="mb-6 inline-flex max-w-max items-center rounded-full border border-brand/40 bg-brand/5 px-3 py-1.5 text-[10px] font-medium uppercase text-brand/90 sm:text-[11px]">
    <span className="mr-1.5 text-brand" aria-hidden>•</span>
    Est. 2017 · Medical technology
  </p>
)
...
function HeadingContent({ repel = false }: { repel?: boolean }) {
  const rw = repel ? { 'data-repel-word': true } : {}
  return (
    <>
      {heroBadge}
      <h1 className={heroTitleClass}>
        <span className="block will-change-transform" {...rw}>Build</span>
        <span className="inline-flex items-start gap-0 text-brand">
          <span className="block will-change-transform" {...rw}>Smarter</span>
        </span>
        <span className="mt-1 block will-change-transform" {...rw}>Grow</span>
        <span className="block will-change-transform" {...rw}>Faster.</span>
      </h1>
      <div className="mt-8 flex max-w-xl items-start gap-5 sm:gap-6">
        <p
          className="font-sans text-[15px] leading-relaxed text-neutral-400 sm:text-base will-change-transform"
          {...rw}
        >
          Mongolia&apos;s leading provider of smart, portable, and AI-powered medical devices and
      ...
      <div className="mt-8 flex items-center gap-3">
        <button
          type="button"
          className="rounded-full bg-brand px-6 py-3 text-sm font-semibold text-neutral-950 transition hover:brightness-105 active:brightness-95 text-nowrap will-change-transform"
          {...rw}
        ...
        <button
          type="button"
          className="rounded-full border border-neutral-600 bg-transparent px-6 py-3 text-sm font-semibold text-neutral-100 transition hover:border-neutral-400 text-nowrap will-change-transform"
          {...rw}
      ...
      <dl className="mt-12 grid grid-cols-3 gap-x-4 gap-y-8 sm:flex sm:flex-wrap  sm:gap-x-32">
        <div className="flex min-w-0 flex-col gap-2 sm:max-w-36 will-change-transform" {...rw}>
          <dd className="font-benzin text-[clamp(1.75rem,5vw,2.5rem)] font-bold leading-none tabular-nums text-white">
            <CountUp className="inline" duration={2} to={20} />+
          </dd>
          <dt className="font-sans text-xs leading-snug text-neutral-500 sm:text-sm">Years of Innovation</dt>
        </div>
        <div className="flex min-w-0 flex-col gap-2 sm:max-w-36 will-change-transform" {...rw}>
          <dd className="font-benzin text-[clamp(1.75rem,5vw,2.5rem)] font-bold leading-none tabular-nums text-white">
            <CountUp className="inline" duration={2} delay={0.08} to={8} />+
          </dd>
          <dt className="font-sans text-xs leading-snug text-neutral-500 sm:text-sm">Main Brands</dt>
        </div>
        <div className="flex min-w-0 flex-col gap-2 sm:max-w-40 will-change-transform" {...rw}>
          <dd className="font-benzin text-[clamp(1.75rem,5vw,2.5rem)] font-bold leading-none tabular-nums text-brand">
            <CountUp className="inline" duration={2.2} delay={0.12} to={3} />K+
          </dd>
          <dt className="font-sans text-xs leading-snug text-neutral-500 sm:text-sm">Devices Deployed</dt>
        </div>
      </dl>
    </>
  )
          ...
          >
            ...
            <g ref={trailMarkRef}>
              <HeroTrailMark
                ...
                label={MOCK_SMART_MODE_PHRASES[0]}
              />
            </g>
          </svg>
