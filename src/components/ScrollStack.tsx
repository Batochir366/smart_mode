...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = '' }) => (
  <div
    className={`scroll-stack-card relative h-80 w-full origin-top rounded-[40px] box-border p-12 shadow-lg shadow-black/40 ring-1 ring-white/10 will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  /** Layout Y in the document / scroller — must NOT use getBoundingClientRect on transformed cards (transform feedback loop). */
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = getLayoutTop(card)
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = getLayoutTop(cardsRef.current[j])
  ...
  const setupNestedLenis = useCallback(() => {
    const scroller = scrollerRef.current
    if (!scroller) return
  ...
  useLayoutEffect(() => {
    ...
    if (useWindowScroll) {
      const removeWindowListeners = setupWindowScroll()
      updateCardTransforms()
      return () => {
  ...
  return (
    <div
      className={
        useWindowScroll
          ? `relative w-full overflow-x-visible overflow-y-visible ${className}`.trim()
    ...
    >
      <div className="scroll-stack-inner min-h-[60vh] px-6 pb-[min(28rem,45vh)] pt-[10vh] md:px-12 lg:min-h-screen lg:px-20">
        {children}
